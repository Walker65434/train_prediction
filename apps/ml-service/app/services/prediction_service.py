...
def fetch_live_train_data(train_id: str):
    API_URL = f"https://api.railradar.in/v1/trains/{train_id}/live"
    headers = {
...
def predict_eta(train_id: str):
    ...
    live_train_info = {
        ...
        "distance_from_origin_km": live_data.get("currentLocation", {}).get("distanceFromOriginKm", 0),
        "distance_from_last_station_km": live_data.get("currentLocation", {}).get("distanceFromLastStationKm", 0),
    ...
    total_dist = float(live_train_info["total_distance_km"]) if live_train_info["total_distance_km"] else 1.0
    dist_origin = float(live_train_info["distance_from_origin_km"]) if live_train_info["distance_from_origin_km"] else 0.0
    next_dist = float(live_train_info["next_station_distance_km"]) if live_train_info["next_station_distance_km"] else 0.0
    ...
    for station in live_data.get("route", []):
        route_array.append({
            ...
            "expectedArrival": station.get("actualArrival", station.get("scheduledArrival", "")),
            "scheduledDeparture": station.get("scheduledDeparture", ""),
            "expectedDeparture": station.get("actualDeparture", station.get("scheduledDeparture", "")),
    ...
    result_payload = {
        ...
        "destination": {
            "stationCode": live_data.get("train", {}).get("destination", {}).get("code", ""),
            "stationName": live_data.get("train", {}).get("destination", {}).get("name", "")
        },
        "currStation": {
        ...
        "location": {
            "latitude": 0.0, # Not provided by api usually
            "longitude": 0.0
        },
