  ...
  return (
    <div className="w-full">
      ...
      {!file ? (
        ...
        <div
          ...
          className={`cursor-pointer border-2 border-dashed rounded-3xl p-6 sm:p-8 text-center transition-all duration-300 ${
            isDragging
              ...
              : 'border-slate-300 bg-white/50 hover:border-cyan-500/50 hover:bg-white/70'
          }`}
        >
          <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-tr from-cyan-500/10 via-indigo-500/10 to-purple-500/10 border border-slate-200 flex items-center justify-center text-cyan-600 shadow-inner group-hover:scale-110 transition-transform">
          ...
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-slate-200 hover:border-cyan-300 hover:bg-slate-50 text-xs font-semibold text-cyan-600 transition-all shadow-sm">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white/70 p-5 backdrop-blur-xl shadow-xl space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            {/* Thumbnail Preview with Laser Sweep Line */}
            <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-2xl overflow-hidden border border-slate-200 flex-shrink-0 bg-slate-100 shadow-inner">
              <img
                src={previewUrl}
                alt="Ticket Preview"
              ...
              {isProcessing && (
                <>
                  ...
                  <div className="absolute inset-x-0 h-1 bg-gradient-to-r from-transparent via-cyan-500 to-transparent shadow-[0_0_12px_#06b6d4] animate-laser" />
                  <div className="absolute inset-0 flex items-center justify-center">
                    <FontAwesomeIcon
                      icon={faSpinner}
                      className="text-cyan-600 text-2xl animate-spin drop-shadow"
                    />
                  </div>
                </>
              )}
            ...
            <div className="flex-1 w-full min-w-0">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button
                  type="button"
                  onClick={handleReset}
                  className="px-2.5 py-1 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 text-xs font-medium transition-colors flex items-center gap-1"
                  title="Remove image"
                >
                ...
                {!isProcessing && extractedNumber && (
                  ...
                  >
                    <div className="flex items-center gap-2 flex-wrap">
                      <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 font-medium text-xs shadow-sm">
                        <FontAwesomeIcon icon={faCheckCircle} />
                        <span>Detected Train:</span>
                        <span className="font-mono font-bold text-sm text-emerald-600 tracking-wider">
                          #{extractedNumber}
                        </span>
                      </div>
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-600 hover:to-indigo-700 text-white text-xs font-bold shadow-md shadow-cyan-500/20 transition-all flex items-center gap-1.5"
                      >
                    ...
                    {candidates.length > 1 && (
                      <div className="text-xs text-slate-500 flex items-center gap-2 flex-wrap pt-1">
                        ...
                        {candidates
                          ...
                          .map((num) => (
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-white border border-slate-200 hover:bg-slate-50 hover:border-slate-300 text-slate-700 font-mono text-xs transition-colors"
                            >
                              #{num}
                            </button>
                          ))}
              ...
              {!isProcessing && errorMsg && (
                <div className="mt-2 text-xs text-amber-700 bg-amber-500/10 border border-amber-500/20 px-3 py-2 rounded-xl flex items-center gap-2">
